...
func _run() -> void:
	...
	settings.confirmation_dialog.get_cancel_button().emit_signal("pressed")
	assert(TestCheckScript.check(cleared == 0, "cancel reset makes no changes"))
	settings.request_clear()
	settings.confirmation_dialog.get_ok_button().emit_signal("pressed")
	...
	assert(TestCheckScript.check(controller.encounter_hud.settings_panel.confirmation_dialog.visible, "clear progress opens confirmation"))
	controller.encounter_hud.settings_panel.confirmation_dialog.get_ok_button().emit_signal("pressed")
