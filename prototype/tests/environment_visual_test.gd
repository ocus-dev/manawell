...
func _run() -> void:
	...
	assert(controller.MACHINE_X == 160.0)
	assert(controller.environment_visual != null)
	assert(controller.environment_visual.is_prepared())
	assert(controller.environment_visual.selected_well_id == "well_1")
	...
	var level_one_texture: Texture2D = controller.environment_visual.backdrop_texture
	controller.environment_visual.set_well_id("well_2")
	assert(controller.environment_visual.selected_well_id == "well_2")
	assert(controller.environment_visual.backdrop_texture != level_one_texture)
	assert(controller.environment_visual.backdrop_texture.get_width() == 1280)
	assert(controller.environment_visual.backdrop_texture.get_height() == 720)
